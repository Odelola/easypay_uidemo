...
function AppScreen({ flex = true, className, children, style, ...props }: { flex?: boolean } & ViewProps) {
    ...
    return (
            <SafeAreaView  className={className} style={[{ paddingTop: paddingFromTop,  paddingHorizontal: paddingHorizontal, flexGrow: flex ? 1 : 0, paddingBottom: paddingFromBottom }, style]} {...props}>
                {children}
            </SafeAreaView>
    )
}
